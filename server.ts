...
import xaviraBookingHandler from "./api/xavira-booking";
...
async function startServer() {
  ...
  for (let i = 2; i < process.argv.length; i += 1) {
    ...
    if (value && !value.startsWith("--")) args.set(key, value);
  }

  const PORT = Number(args.get("--port") ?? process.env.PORT ?? 3000);
  const HMR_PORT = Number(args.get("--hmr-port") ?? process.env.HMR_PORT ?? 24678);

  app.use(express.json());

  app.post("/api/xavira-booking", xaviraBookingHandler);
  ...
  app.post("/api/contact", async (req, res) => {
    try {
      ...
      // Basic spam protections: honeypot + too-fast submit.
      ...
      // Don't leak detection to bots. Just return success.
      ...
      const to = parseEmailList(process.env.CONTACT_TO_EMAILS, [
